...
def scrape_zhihu_keyword(page: Page, keyword: str) -> List[Dict]:
    ...
    time.sleep(2.0)
    logger.info(f"[DEBUG] page.title={page.title()!r}")
    logger.info(f"[DEBUG] page.url={page.url!r}")

    prefix = safe_name(kw)

    # 先保存一次打开后的页面（便于对比正常/异常）
    # 如果你不想每次都存，可以注释掉下一行
    save_debug(page, f"{prefix}_open", reason="After opening search page")

    # 如果明显是验证/登录页，直接返回空（同时 artifacts 里有 debug）
    ...
    try:
        ...
        logger.warning("未检测到搜索结果 DOM，可能DOM变动或被限流（见 debug/）")
