...
# =====================================================================
# ⚙️ 配置区 (终极安全版：通过环境变量读取，代码里零密文)
# =====================================================================

# 自动从 GitHub Actions 的 Secrets 中获取密钥
GITHUB_TOKEN = os.getenv("MY_GITHUB_TOKEN", "")
SS_API_KEY = os.getenv("SS_API_KEY", "")

if not GITHUB_TOKEN or not SS_API_KEY:
    print("⚠️ 警告：未能读取到 API 密钥，请检查 GitHub Secrets 配置！但程序将继续以无密钥模式尝试运行。")
...
def create_robust_session():
    ...
    retry_strategy = Retry(total=8, connect=5, read=5, backoff_factor=3, status_forcelist=[429, 500, 502, 503, 504], allowed_methods=["GET"])
...
def fetch_arxiv(keyword):
    ...
    params = {"search_query": f'all:"{keyword}"', "sortBy": "submittedDate", "sortOrder": "descending", "max_results": 50}
    headers = {"User-Agent": "Mozilla/5.0"}
    papers = []
    try:
        res = http_session.get(url, params=params, headers=headers, timeout=(15, 60))
        if res.status_code == 200:
            ...
            for entry in root.findall('atom:entry', ns):
                ...
                if pub is not None and summ is not None: papers.append({"year": pub.text[:4], "abstract": summ.text.replace('\n', ' ')})
    except: pass
    return papers
...
def fetch_github(keyword):
    ...
    try:
        ...
        if res.status_code == 200:
            for item in res.json().get('items', []): repos.append({"year": item.get('created_at', '')[:4], "abstract": item.get('description', '') or ''})
    except: pass
    return repos
...
def main():
    print("🚀 [启动] 学术热点雷达 - 平台全维解构引擎 (安全秘钥版)")
